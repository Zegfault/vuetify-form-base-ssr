...
const hour = "hour";
const minute = "minute";
const second = "second"; // symbol on drop
...
var script = {
  ...
  methods: {
    ...
    mapTypeToComponent(type) {
      ...
      const allTypeComponents = { ...typeToComponent,
    ...
    bindSchemaText(obj) {
      return { ...defaultPickerSchemaText,
        ...obj.schema.text
      };
    },

    bindSchemaMenu(obj) {
      return { ...defaultPickerSchemaMenu,
        ...obj.schema.menu
      };
    },
    ...
    getKeyClassNameWithAppendix(obj, appendix) {
      ...
      return `${appendix ? appendix + classKeyDelimiter : ""}${this.id ? this.id + classKeyDelimiter : ""}${obj.key.replace(/\./g, "-")}`; // return `${appendix ? appendix + classKeyDelimiter : ''}${obj.key.replace(/\./g, '-')}`
    },
    ...
    getTypeClassNameWithAppendix(obj, appendix) {
      ...
      return `${appendix ? appendix + classKeyDelimiter : ""}${this.id ? this.id + classKeyDelimiter : ""}${obj.schema.type}`; // return `${appendix + classKeyDelimiter}${obj.schema.type}`
    },
    ...
    getGridAttributes(obj) {
      ...
      const colObject = colSchema ? // if available use schema definition of cols
      ...
      const offsetObject = offset ? lodash.isPlainObject(offset) ? offset : {
        offset
      } : offset;
      ...
      const orderObject = order ? lodash.isPlainObject(order) ? order : {
        order
      } : order;
    ...
    onEvent(event = {}, obj, tag) {
      ...
      const index = this.index; // avoid circular JSON in dragstart
      ...
      lodash.delay(() => {
        this.emitValue(event.type, emitObj);
      });
      return emitObj;
    },
    ...
    flattenObjects(dat = {}, sch) {
      const data = {};
      const schema = {}; // Organize Formular using Schema not Data
      ...
      Object.keys(sch).forEach(key => {
        ...
        const datObjectWithoutSchemaType = lodash.isPlainObject(dat[key]) && !sch[key].type;
    ...
    flattenAndCombineToArray(data, schema) {
      ...
      const flattenedObjects = this.flattenObjects(data, schema); // ... and combine them to an array
        ...
        fn: function (ref) {
          ...
          return [_c('v-col', _vm._g(_vm._b({
            directives: [{
              ...
              name: "intersect",
              rawName: "v-intersect",
              value: function (entries, observer) {
